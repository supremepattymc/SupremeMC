...
public class SoulBlazeEntitySMC extends MonsterEntity 
{
	private float allowedHeightOffset = 0.5F;
	private int nextHeightOffsetChangeTick;
	private static final DataParameter<Byte> DATA_FLAGS_ID = EntityDataManager.defineId(SoulBlazeEntitySMC.class, DataSerializers.BYTE);

	public SoulBlazeEntitySMC(EntityType<? extends SoulBlazeEntitySMC> blaze, World world) 
	{
		super(blaze, world);
	...
	{
		this.goalSelector.addGoal(4, new SoulBlazeEntitySMC.FireballAttackGoal(this));
	...
	@Override
	public boolean causeFallDamage(float x, float y) 
	{
		return false;
	}
	...
	{
		private final SoulBlazeEntitySMC blaze;
		private int attackStep;
		private int attackTime;
		private int lastSeen;

		public FireballAttackGoal(SoulBlazeEntitySMC blaze) 
		{
			this.blaze = blaze;
				...
				else if(d0 < this.getFollowDistance() * this.getFollowDistance() && flag) 
				{
					...
					if(this.attackTime <= 0) 
					{
						...
						if(this.attackStep > 1) 
						{
							...
							if(!this.blaze.isSilent()) this.blaze.level.levelEvent((PlayerEntity)null, 1018, this.blaze.blockPosition(), 0);
							for(int i = 0; i < 1; ++i) 
							{
								SoulFireballEntitySMC smallfireballentity = new SoulFireballEntitySMC(this.blaze.level, this.blaze, d1 + this.blaze.getRandom().nextGaussian() * (double)f, d2, d3 + this.blaze.getRandom().nextGaussian() * (double)f);
