package source.suprememc.tileentity.brewingstand;
...
import source.suprememc.recipes.BrewingRecipeRegistrySMC;
import source.suprememc.recipes.PotionBrewingSMC;

public class BrewingStandTileEntitySMC extends LockableTileEntity implements ISidedInventory, ITickableTileEntity 
{
	...
	protected final IIntArray dataAccess = new IIntArray() 
	{
		public int get(int p_221476_1_) {
			switch(p_221476_1_) {
			case 0:
				return BrewingStandTileEntitySMC.this.brewTime;
			case 1:
				return BrewingStandTileEntitySMC.this.fuel;
			default:
				return 0;
			}
		}
		...
		public void set(int p_221477_1_, int p_221477_2_) {
			switch(p_221477_1_) {
			case 0:
				BrewingStandTileEntitySMC.this.brewTime = p_221477_2_;
				break;
			case 1:
				BrewingStandTileEntitySMC.this.fuel = p_221477_2_;
			}
	...
	public BrewingStandTileEntitySMC() {
		super(SMCTileEntities.BREWING_STAND);
	}

	protected ITextComponent getDefaultName() {
		return new TranslationTextComponent("container.suprememc.brewing");
	}
	...
	public void tick() {
		...
		if (this.fuel <= 0 && itemstack.getItem() == SMCItems.SOUL_BLAZE_POWDER) {
			this.fuel = 20;
			itemstack.shrink(1);
			this.setChanged();
		}
		...
		if (!this.level.isClientSide) {
			...
			if (!Arrays.equals(aboolean, this.lastPotionCount)) {
				...
				if (!(blockstate.getBlock() instanceof SoulBrewingStandSMC)) {
					return;
				}

				for(int i = 0; i < SoulBrewingStandSMC.HAS_BOTTLE.length; ++i) {
					blockstate = blockstate.setValue(SoulBrewingStandSMC.HAS_BOTTLE[i], Boolean.valueOf(aboolean[i]));
				}
	...
	private boolean isBrewable() {
		...
		if (!itemstack.isEmpty()) return BrewingRecipeRegistrySMC.canBrew(items, itemstack, SLOTS_FOR_SIDES); // divert to VanillaBrewingRegistry
		if (itemstack.isEmpty()) {
			return false;
		} else if (!PotionBrewingSMC.isIngredient(itemstack)) {
			return false;
		} else {
			for(int i = 0; i < 3; ++i) {
				ItemStack itemstack1 = this.items.get(i);
				if (!itemstack1.isEmpty() && PotionBrewingSMC.hasMix(itemstack1, itemstack)) {
					return true;
				}
			}
	...
	private void doBrew() {
		...
		BrewingRecipeRegistrySMC.brewPotions(items, itemstack, SLOTS_FOR_SIDES);
	...
	public boolean canPlaceItem(int p_94041_1_, ItemStack p_94041_2_) {
		if (p_94041_1_ == 3) {
			return BrewingRecipeRegistrySMC.isValidIngredient(p_94041_2_);
		} else {
			Item item = p_94041_2_.getItem();
			if (p_94041_1_ == 4) {
				return item == SMCItems.SOUL_BLAZE_POWDER;
			} else {
				return BrewingRecipeRegistrySMC.isValidInput(p_94041_2_) && this.getItem(p_94041_1_).isEmpty();
			}
		}
	}
	...
	protected Container createMenu(int p_213906_1_, PlayerInventory p_213906_2_) {
		return new SupremeBrewingStandContainerSMC(p_213906_1_, p_213906_2_, this, this.dataAccess);
	}
