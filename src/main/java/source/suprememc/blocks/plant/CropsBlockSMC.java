...
public class CropsBlockSMC extends BushBlock implements IGrowable 
{
	...
	public CropsBlockSMC(String name, Item seeds) 
	{
	...
	protected static float getGrowthSpeed(Block blockIn, IBlockReader worldIn, BlockPos pos) 
	{
		...
		for(int i = -1; i <= 1; ++i) 
		{
			for(int j = -1; j <= 1; ++j) 
			{
				...
				{
					f1 = 1.0F;
					if(blockstate.isFertile(worldIn, pos.offset(i, 0, j))) f1 = 3.0F; 
				}
	            if (i != 0 || j != 0) f1 /= 4.0F;
	            f += f1;
			}
	...
	{
		if(entityIn instanceof RavagerEntity && net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(worldIn, entityIn)) worldIn.destroyBlock(pos, true, entityIn);
